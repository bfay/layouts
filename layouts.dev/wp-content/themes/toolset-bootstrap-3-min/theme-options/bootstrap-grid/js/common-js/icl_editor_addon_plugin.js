...
jQuery(document).ready(function(){
     ...
     */
    jQuery(document).on('click', '.js-wpv-v-icon img, .js-code-editor-toolbar-button-v-icon', function(e){
    ...
    jQuery('.editor-addon-top-link').on('click', function(){
        ...
        var target = jQuery(this).parents('li').find('.'+jQuery(this).data('editor_addon_target')+'-target');
...
function icl_editor_bind_auto_close() {
    ...
    jQuery(document).on('click',function(e){
        ...
        if ( $target.hasClass('wpv_add_fields_button') ) {
        // Do nothing other dropdowns will take care
        } else if ($target.parents('.editor_addon_dropdown').length < 1) {
...
var icl_editor = (function(window, $){
	...
	function codeMirrorCursorIsWithin( area, tStart, tEnd )
	{
		...
		if( !codemirror )
		{
			console.log("You are not in a code mirror text area");
			return false;
		}
